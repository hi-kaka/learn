...
class FilterRules():
    ...
    def run(self):
        ...
        self.init_uniq_key(self.all_lst)
        ...
        for key_obj in key_obj_list:
            ...
            if sn_key and sn_key != u'' and sn_key != 'NotSpecified':
                if sn_key not in self.key_dic_map.keys():
                    self.key_dic_map[sn_key] = [key_obj.ip,key_obj.ssh_port,key_obj.ssh_user,key_obj.ssh_passwd,key_obj.ssh_rsa,key_obj.rsa_pass,key_obj.system_ver,key_obj.hostname,key_obj.ssh_status,key_obj.ssh_type,key_obj.mac_address,sn_key,key_obj.mathine_type]
            elif mac_addr:
                if mac_addr not in self.key_dic_map.keys():
                    self.key_dic_map[mac_addr] = [key_obj.ip,key_obj.ssh_port,key_obj.ssh_user,key_obj.ssh_passwd,key_obj.ssh_rsa,key_obj.rsa_pass,key_obj.system_ver,key_obj.hostname,key_obj.ssh_status,key_obj.ssh_type,key_obj.mac_address,key_obj.sn,key_obj.mathine_type]
            else:
                print u"error:no sn and no mac:%s"%key_obj.id
                logger.error("error:no sn and no mac,id is :%s"%key_obj.id)
        print 'self.key_dic_map:',self.key_dic_map
    ...
    def filter_type(self):
        ...
        PhysicalServerInfo.objects.create(id=1,server_ip='0.0.1.1',machine_brand='test',system_ver='test',sys_hostname='test',mac='test',sn='test',vir_type=2,ser_cabin_id=2,conn_phy_id=1)
        for item,item_val in self.key_dic_map.items():
            ...
            if re.search("KVM",item_val[-1]) or re.search("QEMU",item_val[-1]):
                ...
                if sn_key and sn_key != 'NotSpecified':
                    PhysicalServerInfo.objects.create(server_ip=item_val[0],machine_brand=item_val[-1],system_ver=item_val[6],sys_hostname=item_val[7],mac=item_val[-3],sn=item_val[-2],ser_cabin_id=2,conn_phy_id=int(self.key_sn_relate[sn_key]))
                else:
                    PhysicalServerInfo.objects.create(server_ip=item_val[0],machine_brand=item_val[-1],system_ver=item_val[6],sys_hostname=item_val[7],mac=item_val[-3],sn=item_val[-2],ser_cabin_id=2,conn_phy_id=int(self.key_sn_relate[mac_addr]))
